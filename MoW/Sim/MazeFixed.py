...
ax.add_patch(
    Rectangle((start_x * BLOCK + 0.05, start_y * BLOCK + 0.05), BLOCK - 0.1, BLOCK - 0.1, facecolor='green', alpha=0.5))
ax.add_patch(
    Rectangle((end_x * BLOCK + 0.05, end_y * BLOCK + 0.05), BLOCK - 0.1, BLOCK - 0.1, facecolor='red', alpha=0.5))
...
dead_ends = set()  # Cells to never revisit
last_move = None
stuck_counter = 0
...
while current_x != end_x or current_y != end_y:
    ...
    # Dead end detection: only 1 option and it's going back
    if len(move_options) == 1 and move_options[0][2]:  # only reverse available
        dead_ends.add((current_x, current_y))
        print(f"  Dead end detected at ({current_x},{current_y})")

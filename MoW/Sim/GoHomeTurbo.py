"""
D* Lite maze navigation - goes to end, returns home, then FAST optimal path
Phase 3 uses continuous flight through waypoints (no stopping at each block)
"""
...
title = ax.set_title('D* Lite Maze Navigation - Phase 1: Exploring to End')
...
# Draw start/end
start_patch = ax.add_patch(Rectangle((start_x * BLOCK + 0.05, start_y * BLOCK + 0.05),
                                     BLOCK - 0.1, BLOCK - 0.1, facecolor='green', alpha=0.5))
end_patch = ax.add_patch(Rectangle((end_x * BLOCK + 0.05, end_y * BLOCK + 0.05),
...
optimal_line, = ax.plot([], [], 'g-', linewidth=4, alpha=0.7, label='Optimal path')
fast_line, = ax.plot([], [], 'orange', linewidth=3, linestyle='--', alpha=0.8, label='Fast path')
detected_walls = set()
...
def update_drone_position_smooth(bx, by):
    """Update drone marker position for visualization"""
    drone_pos.set_data([bx * BLOCK + BLOCK / 2], [by * BLOCK + BLOCK / 2])
    plt.draw()
    plt.pause(0.05)
...
print(f"✓ Reached end in {steps} steps!")
time.sleep(1)

# ============ PHASE 2: Return Home ============
print("\n=== PHASE 2: Returning Home ===")
title.set_text('D* Lite Maze Navigation - Phase 2: Returning Home')
...
update_plot({}, current_x, current_y)
print(f"✓ Returned home in {return_steps} steps!")
time.sleep(1)

# ============ PHASE 3: FAST Optimal Path to End ============
print("\n=== PHASE 3: FAST Optimal Path to End ===")
title.set_text('D* Lite Maze Navigation - Phase 3: FAST Continuous Flight!')
...
if optimal_path:
    ...
    opt_x = [p[0] * BLOCK + BLOCK / 2 for p in optimal_path]
    opt_y = [p[1] * BLOCK + BLOCK / 2 for p in optimal_path]
    optimal_line.set_data(opt_x, opt_y)

    # Simplify path to only corners/waypoints
    waypoints = simplify_path(optimal_path)

    # Show simplified fast path
    fast_x = [p[0] * BLOCK + BLOCK / 2 for p in waypoints]
    fast_y = [p[1] * BLOCK + BLOCK / 2 for p in waypoints]
    fast_line.set_data(fast_x, fast_y)

    # Mark waypoints
    for wp in waypoints:
        ax.plot(wp[0] * BLOCK + BLOCK / 2, wp[1] * BLOCK + BLOCK / 2,
                'o', color='orange', markersize=10, zorder=6)

    print(f"Full optimal path ({len(optimal_path)} points): {optimal_path}")
    ...
    for i, waypoint in enumerate(waypoints[1:], 1):  # Skip first (current position)
        ...
        print(f"Flying to waypoint {i}/{len(waypoints) - 1}: ({wp_x},{wp_y}) at max speed")

        # Fly at maximum speed to next waypoint
        api.single_fly_straight_flight(abs_x, abs_y, FLIGHT_HEIGHT, MAX_SPEED)
        ...
        update_drone_position_smooth(current_x, current_y)
        path_line.set_data([x * BLOCK + BLOCK / 2 for x in path_x],
                           [y * BLOCK + BLOCK / 2 for y in path_y])

        time.sleep(0.1)  # Minimal delay for visualization
...
title.set_text(f'Complete! Fast path: {len(waypoints) - 1} commands vs {len(optimal_path) - 1} blocks')
